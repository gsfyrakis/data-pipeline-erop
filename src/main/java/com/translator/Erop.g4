...
lhs
   : eventmatch (WS constraint)*
   ;

eventmatch
   : alphanum WS MATCHES WS BRA roleplayerConstraint (COMMA roleplayerConstraint)* KET
   ;
...
rpConstraint
   : alphanum DOT ('rights' | 'obligs') WS?
   ;
   //roleplayerConstraint: alphanum DOT (BOTYPE | ORIGINATOR | RESPONDER) WS? (EQUALS|NOTEQ) WS? alphanum;
   
roleplayerConstraint
   : ((ORIGINATOR | RESPONDER | OUTCOME) WS? (EQUALS | NOTEQ) WS? alphanum)
   | ((BOTYPE) WS? (EQUALS | NOTEQ) WS? upalphanum)
   ;
...
timePartialComparison
   ...
   | alphanum DOT DAY WS? (IN | NOTIN) WS? SQUAREBRA WEEKDAY DOT DOT WEEKDAY SQUAREKET
   | alphanum DOT DATE WS? (EQUALS | NOTEQ | BEFORE | AFTER) WS? DIGIT DIGIT
   | alphanum DOT DATE WS? (IN | NOTIN) WS? SQUAREBRA DIGIT DIGIT DOT DOT DIGIT DIGIT SQUAREKET
   | alphanum DOT MONTH WS? (EQUALS | NOTEQ | BEFORE | AFTER) WS? MONTHID
   | alphanum DOT MONTH WS? (IN | NOTIN) WS? SQUAREBRA MONTHID DOT DOT MONTHID SQUAREKET
   | alphanum DOT YEAR WS? (EQUALS | NOTEQ | BEFORE | AFTER) WS? DIGIT DIGIT DIGIT DIGIT
   | alphanum DOT MONTH WS? (IN | NOTIN) WS? SQUAREBRA DIGIT DIGIT DIGIT DIGIT DOT DOT DIGIT DIGIT DIGIT DIGIT SQUAREKET
   ;

historicalQuery
   : (HAPPENED | CTHAPPENED) WS? BRA upalphanum COMMA WS? alphanum COMMA WS? alphanum COMMA WS? genericString COMMA WS? outcome KET
   ;
   ////ropConstraint: upalphanum WS IN | NOTIN WS alphanum DOT ropset;
   
ropConstraint
   : upalphanum WS IN
   | NOTIN WS alphanum DOT ropset
   ;

   //// Right hand side structure
rhs
   : rhsaction (WS? rhsaction)*
   ;

rhsaction
   // : addaction | remaction | termaction | passaction SEMICOLON;
   : (ifstatement | termaction | passaction | addaction | remaction) WS?
   ; // WS? SEMICOLON;
   
   //// support for if-then-else-endif statement
ifstatement
   : IF WS condition WS rhs WS (ELSE WS rhs WS)? ENDIF
   ;
...
addactionNotime
   : alphanum DOT ropset WS? ADDROP WS? upalphanum
   ;
...
remactionNotime
   : alphanum DOT ropset WS? REMROP WS? upalphanum
   ;
...
absoluteTime
   : DQUOTE DIGIT DIGIT DASH DIGIT DIGIT DASH DIGIT DIGIT DIGIT DIGIT WS DIGIT DIGIT COLON DIGIT DIGIT COLON DIGIT DIGIT DQUOTE
   ;
...
relTimeElement
   : DQUOTE DIGIT+ ('hour' | 'day' | 'minute' | 'month' | 'year') DQUOTE
   ;
...
genericString
   : DQUOTE (LOWER | UPPER | DIGIT | WS | SEMICOLON | COLON | COMMA | QUOTE | DOT | DASH | BACKSLASH)* DQUOTE
   ;
